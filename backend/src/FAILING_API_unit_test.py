...
class FlaskTestCase(unittest.TestCase):
    ...
    @patch.object(System, 'get_exam_results')
    def test_get_exam_results_success(self, mock_get_exam_results):
        mock_get_exam_results.return_value = {
            'examId': 33025,
            'testType': 'Blood Test Iron',
            'result': 'Normal'
        }
        response = self.app.get('/exam/results/33025')
        self.assertEqual(response.status_code, 200)
        self.assertEqual(response.json['result'], 'Normal')
